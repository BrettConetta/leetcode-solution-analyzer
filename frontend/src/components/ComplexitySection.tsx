...
const GRID = "grid grid-cols-[4rem_1fr_1fr_5.5rem] gap-x-4 gap-y-3 items-baseline";

export function ComplexitySection({ time, space }: ComplexitySectionProps) {
    const showSpaceTradeoffNote =
        time.isOptimal &&
        space.isOptimal &&
        space.actual !== space.optimal;
    return (
    <div className="rounded-md border border-zinc-700 bg-zinc-800/50 p-4">
        ...
        {showSpaceTradeoffNote && (
            <p className="mt-3 text-sm text-zinc-500">
                Space is optimal for this time complexity. A lower-space solution exists
                with worse time.
            </p>
)}
    </div>
