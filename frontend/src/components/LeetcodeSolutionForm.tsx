...
export function LeetcodeSolutionForm({ onSubmit, isSubmitting, submitError = null }: LeetcodeSolutionFormProps) {
    ...
    return (
        <form className="space-y-4" onSubmit={handleSubmit}>
          <div >
            <label htmlFor="problemId" className={labelClassName}>Problem ID</label>
            <input 
              type="text" 
              inputMode="numeric"
              id="problemId" 
          ...
          <div className="max-w-fit">
            <label htmlFor="codeLanguage" className={labelClassName}>Code Language</label>
            <select id="codeLanguage" name="codeLanguage" className="mt-1 block w-full rounded-md border-2 border-zinc-700 bg-zinc-800 text-white focus:border-sky-500 focus:ring-sky-500 focus:outline-none focus:ring-1">
              {CODE_LANGUAGES.map((language) => (
                <option key={language} value={language}>{formatLanguageLabel(language)}</option>
              ))}
            </select>
          </div>
          <div>
            <label htmlFor="userCode" className={labelClassName}>User Code</label>
            <ResizableTextarea 
              id="userCode" 
              name="userCode"
              defaultRows={10}
